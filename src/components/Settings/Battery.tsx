...
import { Zap, Leaf, Activity, Rocket, Flame } from 'lucide-react';
...
export default function Battery() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If telemetry fails, we leave it as null rather than using fake fallback data
        const telData: PowerTelemetry = await invoke('get_power_telemetry').catch(() => null);
        setBattery(batData);
  ...
  return (
    <div className="flex flex-col gap-[1.5rem] h-full pb-[1.5rem] pt-2 overflow-y-auto custom-scrollbar text-slate-800 dark:text-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          {battery ? (
            <>
              ...
              <div className="relative flex items-center justify-center w-[8rem] h-[8rem] rounded-full bg-slate-200/50 dark:bg-slate-950/80 border-[0.2rem] border-white dark:border-white/10 shadow-inner overflow-hidden mb-[1rem] group-hover:scale-105 transition-transform duration-500">
                <div 
                  className={`absolute bottom-0 w-full bg-gradient-to-t ${getLiquidColor(battery.percentage, isCharging || false)} transition-all duration-1000 ease-in-out opacity-90`}
        ...
        <div className="relative overflow-hidden rounded-[1.5rem] bg-white/60 dark:bg-zinc-900/60 backdrop-blur-3xl border border-slate-200 dark:border-white/10 shadow-sm p-[1.5rem] flex flex-col justify-between hover:shadow-[0_0.5rem_1.5rem_rgba(0,0,0,0.1)] transition-all duration-500 group">
          ...
          <div className="my-auto py-[0.5rem]">
            <div className="text-[2.2em] font-black font-mono tracking-tight text-slate-900 dark:text-white">
              {telemetry ? telemetry.current_wattage.toFixed(1) : '--'} <span className="text-[0.4em] font-sans font-semibold text-slate-500 uppercase tracking-widest">Watts</span>
            </div>
            <p className="text-[0.75em] text-slate-500 dark:text-slate-400 mt-[0.25rem]">System voltage draw rate</p>
          </div>
          ...
          <div className="pt-[0.75rem] border-t border-slate-200/80 dark:border-white/5 flex justify-between items-center text-[0.8em] font-medium text-slate-600 dark:text-slate-300">
            ...
            <span className="font-mono font-bold text-blue-500 bg-blue-500/10 px-[0.5rem] py-[0.15rem] rounded-[0.4rem]">{telemetry ? telemetry.session_draw_mah : '--'} mAh</span>
          </div>
        </div>
