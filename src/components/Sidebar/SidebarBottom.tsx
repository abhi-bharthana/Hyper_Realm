...
export function SidebarBottom() {
  ...
  return (
    <div className={`mt-3 rounded-2xl bg-white/50 dark:bg-black/50 border border-white/40 dark:border-white/5 flex items-center transition-all duration-[400ms] overflow-hidden ${
      isSidebarCollapsed ? 'justify-center w-9 h-9 self-center p-0 shrink-0' : 'space-x-3 w-full p-3 shrink-0'
    }`}>
      ...
      {/* 🔥 FIX: absolute on collapse so it doesn't offset the dot */}
      <div className={`overflow-hidden whitespace-nowrap transition-all duration-[400ms] ease-in-out ${isSidebarCollapsed ? 'absolute opacity-0 w-0 pointer-events-none' : 'relative w-[120px] opacity-100 ml-1'}`}>
